<div class="max-w-xl mx-auto w-full">
    <form action="#" method="get" class="bg-element p-6 space-y-4 rounded-lg mb-8" id="contact-form">
        ...
        <div>
            <label for="phone" id="phone">{{ __('welcome.phone') }}</label>
            <input type="tel" id="phone" name="phone" placeholder="+32 456789011"
                   class="mt-1 w-full bg-background rounded-lg pl-2 font-text">
        </div>
        <div>
            <label for="message" id="message">{{ __('welcome.message') }}</label>
        ...
        <div class="flex justify-center bg-cta rounded-lg pt-2 pb-2 hover:bg-hover">
            <button type="submit" class="text-white hover:bg-hover font-text cursor-pointer ">{{ __('welcome.sent') }}</button>
        </div>
    </form>
</div>
