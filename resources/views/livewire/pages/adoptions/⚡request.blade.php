...
new class extends Component {
    ...
    public function openAdoption(int $id): void
    {
        $request = Adoption::findOrFail($id);

        $this->selectedRequest = $request;

        $this->dispatch('open-adoption-modal');
    }
};
?>

<div class="col-span-12 md:col-span-6">
    <form wire:submit.prevent="submit" class="bg-element p-4 md:p-6 space-y-4 rounded-lg mb-8" id="requestForm">
        <div class="flex flex-col md:flex-row justify-around gap-4">
            <div class="w-full">
                <label for="name" id="name">Nom</label>
                <input wire:model="name" type="text" name="name" id="name" placeholder="Dupont"
                       class="mt-1 w-full bg-background rounded-lg pl-2 font-text">
            </div>
            <div class="w-full">
                <label for="firstName" id="firstName">Prénom</label>
                <input wire:model="firstName" type="text" name="firstName" id="firstName" placeholder="Jean"
                       class="mt-1 w-full bg-background rounded-lg pl-2 font-text">
            </div>
        </div>
        <div>
        ...
        <div class="flex justify-center bg-cta rounded-lg pt-2 pb-2 hover:bg-hover">
            <button type="submit" class="text-white hover:bg-hover font-text cursor-pointer ">Envoyez votre demande
                d'adoption
            </button>
        </div>
    </form>
    @if( session('success') )
        <div
            class="max-w-4xl mx-auto p-4 mb-4 text-green-700 bg-green-100 border border-green-200 rounded animate-fade-up-out">
            {{ session('success') }}
        </div>
    @endif
